...
source_pywal() {
  ...
  altbackground="$(pastel color "$background" | pastel lighten 0.10 | pastel format hex)"
  altforeground="$(pastel color "$foreground" | pastel darken 0.30 | pastel format hex)"
  # --- CORRECTED ASSIGNMENT FOR modbackground ---

  # 1. Pipe the output of pastel gradient directly to readarray
  #    -t option removes the trailing newline from each element
  readarray -t modbackground < <(pastel gradient -n 3 "$background" "$altbackground" | pastel format hex)

  # --- DEBUGGING modbackground (now correct) ---
  echo "DEBUG: modbackground elements (using readarray):"
  if [ "${#modbackground[@]}" -eq 0 ]; then
    echo "ERROR: modbackground array is empty. Check pastel command and input variables."
    # You might want to print the raw output here for more debugging:
    # echo "Raw pastel output: $(pastel gradient -n 3 "$background" "$altbackground" | pastel format hex)"
  else
    for i in "${!modbackground[@]}"; do
      echo "  modbackground[$i]: '${modbackground[$i]}'"
    done
...
apply_ghostty() {
  # ghostty : colors
  cat >"$PATH_GHOSTTY"/colors <<-_EOF_
		# Colors configuration (generated by theme.sh)
		background = ${background}
		foreground = ${foreground}
